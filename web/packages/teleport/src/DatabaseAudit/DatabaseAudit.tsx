...
import EventList from './DatabaseQueryEventList';
import useDatabaseAuditEvents, { State } from './useDatabaseAuditEvents';

export function DatabaseAuditContainer() {
  const teleCtx = useTeleport();
  const { clusterId } = useStickyClusterId();
  const state = useDatabaseAuditEvents(teleCtx, clusterId);
  return <DatabaseAudit {...state} />;
}

export function DatabaseAudit(props: State) {
  const {
    attempt,
    range,
    setRange,
  ...
  return (
    <FeatureBox>
      <FeatureHeader alignItems="center">
        <FeatureHeaderTitle mr="8">Database Query Audit</FeatureHeaderTitle>
        <RangePicker
          ml="auto"
          range={range}
          ranges={rangeOptions}
