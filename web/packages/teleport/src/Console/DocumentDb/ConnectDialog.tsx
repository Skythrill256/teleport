...
function ConnectForm(props: {
  ...
  // Check for configured db_name and db_user in labels
  const getLabelValue = (key: string): string | undefined => {
    const label = props.db.labels?.find(l => l.name === key);
    return label?.value;
  };

  const configuredDbName = getLabelValue('teleport.dev/db_name');
  const configuredDbUser = getLabelValue('teleport.dev/db_user');
  ...
  // Use configured values if available, otherwise use first option
  const getInitialDbName = (): Option | undefined => {
    if (configuredDbName) {
      return { value: configuredDbName, label: configuredDbName };
    }
    return dbNamesOpts?.[0];
  };

  const getInitialDbUser = (): Option | undefined => {
    if (configuredDbUser) {
      return { value: configuredDbUser, label: configuredDbUser };
    }
    return dbUserOpts?.[0];
  };

  const [selectedName, setSelectedName] = useState<Option | undefined>(
    getInitialDbName()
  );
          ...
          <DialogContent flex="0 0 auto">
            {dbNameReq !== 'unsupported' && (
              <ConnectionField
                ...
                isDisabled={!!configuredDbName}
                helperText={
                  configuredDbName
                    ? `Using configured database name: ${configuredDbName}`
                    : null
                }
                rule={
                  dbNameReq === 'required'
            ...
            <ConnectionField
              ...
              isDisabled={props.db.autoUsersEnabled || !!configuredDbUser}
              helperText={
                props.db.autoUsersEnabled
                  ? 'Using auto provisioned user, you cannot change the database user.'
                  : configuredDbUser
                  ? `Using configured database user: ${configuredDbUser}`
                  : null
              }
            />
