...
func (c *connector) getConnectConfig(ctx context.Context) (*pgconn.Config, error) {
	...
	// Check for aws_sm authentication type first (stored in labels)
	if authType, ok := c.database.GetStaticLabels()["teleport.dev/auth_type"]; ok && authType == "aws_sm" {
		secretID, _ := c.database.GetStaticLabels()["teleport.dev/auth_aws_secret_id"]
		region, _ := c.database.GetStaticLabels()["teleport.dev/auth_aws_region"]
		mode, _ := c.database.GetStaticLabels()["teleport.dev/auth_aws_mode"]
		fieldsJSON, _ := c.database.GetStaticLabels()["teleport.dev/auth_aws_fields"]

		if secretID == "" || region == "" {
			return nil, trace.BadParameter("aws_sm auth type requires secret_id and region")
		}
		...
		if fieldsJSON != "" {
			if err := json.Unmarshal([]byte(fieldsJSON), &fields); err != nil {
				return nil, trace.Wrap(err, "failed to parse auth_aws_fields JSON")
			}
		}

		// Fetch password from AWS Secrets Manager
		password, err := libaws.GetSecret(ctx, region, secretID, mode, fields)
		if err != nil {
			return nil, trace.Wrap(err, "failed to get secret from AWS Secrets Manager")
		}
		config.Password = password
		// For aws_sm auth, disable TLS as per requirements
		config.TLSConfig = nil
		c.log.DebugContext(ctx, "Using AWS Secrets Manager authentication", "secret_id", secretID, "region", region)
		return config, nil
	}
