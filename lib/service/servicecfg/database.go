...
type Database struct {
	...
	// Auth contains database authentication configuration.
	Auth DatabaseAuth
	// DBName is the default database name to use when connecting.
	DBName string
	// DBUser is the default database user to use when connecting.
	DBUser string
}
...
func (d *Database) CheckAndSetDefaults() error {
	...
	// Validate aws_sm auth configuration
	if d.Auth.Type == "aws_sm" {
		if d.Auth.AWS.SecretID == "" {
			return trace.BadParameter("aws_sm auth type requires secret_id")
		}
		if d.Auth.AWS.Region == "" {
			return trace.BadParameter("aws_sm auth type requires region")
		}
		if d.Auth.AWS.Mode != "" && d.Auth.AWS.Mode != "string" && d.Auth.AWS.Mode != "json" {
			return trace.BadParameter("aws_sm auth mode must be 'string' or 'json'")
		}
	}
...
func (d *Database) ToDatabase() (types.Database, error) {
	// Copy static labels and add auth configuration if present
	labels := make(map[string]string)
	for k, v := range d.StaticLabels {
		labels[k] = v
	}
	// Store auth configuration in labels for retrieval by connectors
	if d.Auth.Type != "" {
		labels["teleport.dev/auth_type"] = d.Auth.Type
		if d.Auth.AWS.SecretID != "" {
			labels["teleport.dev/auth_aws_secret_id"] = d.Auth.AWS.SecretID
			labels["teleport.dev/auth_aws_region"] = d.Auth.AWS.Region
			labels["teleport.dev/auth_aws_mode"] = d.Auth.AWS.Mode
			// Store fields as JSON if present
			if len(d.Auth.AWS.Fields) > 0 {
				fieldsJSON, err := json.Marshal(d.Auth.AWS.Fields)
				if err == nil {
					labels["teleport.dev/auth_aws_fields"] = string(fieldsJSON)
				}
			}
		}
	}
	// Store default db_name and db_user in labels for retrieval by CLI/UI
	if d.DBName != "" {
		labels["teleport.dev/db_name"] = d.DBName
	}
	if d.DBUser != "" {
		labels["teleport.dev/db_user"] = d.DBUser
	}
...
// DatabaseAuth contains database authentication configuration.
type DatabaseAuth struct {
	// Type is the authentication type (e.g., "aws_sm" for AWS Secrets Manager).
	Type string
	// AWS contains AWS-specific authentication settings.
	AWS DatabaseAuthAWS
}

// DatabaseAuthAWS contains AWS-specific authentication settings for Secrets Manager.
type DatabaseAuthAWS struct {
	// Region is the AWS region where the secret is stored.
	Region string
	// SecretID is the identifier of the secret in AWS Secrets Manager.
	SecretID string
	// Mode specifies how to interpret the secret: "string" for plain text, "json" for JSON structure.
	Mode string
	// Fields is a map of JSON field names to extract when mode is "json".
