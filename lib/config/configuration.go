...
func applyDatabasesConfig(fc *FileConfig, cfg *servicecfg.Config) error {
	...
	for _, database := range fc.Databases.Databases {
		...
		db := servicecfg.Database{
			...
			Auth: servicecfg.DatabaseAuth{
				Type: database.Auth.Type,
				AWS: servicecfg.DatabaseAuthAWS{
					Region:   database.Auth.AWS.Region,
					SecretID: database.Auth.AWS.SecretID,
					Mode:     database.Auth.AWS.Mode,
					Fields:   database.Auth.AWS.Fields,
				},
			},
