...
func (d *databaseInfo) checkAndSetDefaults(cf *CLIConf, tc *client.TeleportClient) error {
	...
	// Check for configured db_name and db_user in database labels first
	if d.Username == "" {
		if dbUser, ok := db.GetStaticLabels()["teleport.dev/db_user"]; ok && dbUser != "" {
			d.Username = dbUser
			logger.DebugContext(cf.Context, "Using configured database user from resource", "database_user", dbUser)
		}
	}
	if d.Database == "" {
		if dbName, ok := db.GetStaticLabels()["teleport.dev/db_name"]; ok && dbName != "" {
			d.Database = dbName
			logger.DebugContext(cf.Context, "Using configured database name from resource", "database_name", dbName)
		}
	}
